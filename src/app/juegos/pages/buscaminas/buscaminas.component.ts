...
export class BuscaminasComponent {
  // Variables
	 ...
	 */
	PreparePlantilla(arrayPiezas:number[]) {
	 ...
	 * @param numPiezas Array con las piezzas
	 * @returns {Array} Devuelve un array de 2 dimensiones con todas los valores resultantes
	 */
	checkBombs(arrayPiezas:number[], numPiezas:number) {
 ...
 */
 function ControladorCelda(this:any) {
	...
	if (arrayPiezzasGlobal[numeroPieza] == -1) {
      ...
      //@ts-ignore
      document.getElementById("piece" + listadoBombas[a]).style.backgroundImage = "url(\"assets/images/explosion.gif\")";
      //@ts-ignore
